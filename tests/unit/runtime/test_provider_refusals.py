...
Two failures that looked like one on a live host. The provider answered every
attempt with a 400 ``invalid_request_error`` — the client was too old for the
model — and the loop retried it as if it were a blip. Then, with the retries
spent and no word written, the run completed "on its preserved answer": the
answer it found was the PREVIOUS run's reply, which the host had handed the
engine as plain session history. The operator saw nothing at all.

So this file holds both halves apart:

* a failure the adapter marks as permanent is not retried in place and does
  not wind down against the endpoint that refused it; it may still step to the
  next model of the chain, which is bounded and one-way;
* "answer preserved" means an answer written in the round that failed, never
  prose from before the operator's latest message, whether or not the host
  tagged that prose as seeded.
"""
