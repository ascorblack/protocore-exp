...
- `runtime/context/budgets.py` — `derive_budgets` превращает один снимок RC во
  ...
  значение; аварийный обрыв держится строго выше него.
...
- `runtime/context/compaction.py` — три прохода по транскрипту, по порядку,
  ...
  **Tier 1** заменяет вышедший за бюджет результат инструмента плейсхолдером,
  ...
  что именно было сброшено. Он также может убрать устаревший reasoning или
  вышедший за бюджет замороженный reference-блок, сохраняя metadata сообщения,
  включая seed-provenance. Обычные **Tier 2** и **Tier 3** не меняют seed-ходы
  прошлых прогонов. **Tier 2** суммаризует старые ходы через
  ...
  дословно. При реактивном восстановлении после переполнения провайдера seed-only отрезки
  можно суммаризовать и сворачивать, оставляя дословно лишь
  `compaction_force_keep_recent_turns` последних сообщений (по умолчанию одно); тег
  seed переносится на каждую замену, а границы seed/current разделяют отрезки,
  поэтому фильтр сохранения хоста не смешивает прогоны. Замороженные
  compaction-reference блоки остаются защищёнными. Свёртка — такое же саммари,
  как любое другое, поэтому более поздняя свёртка поглотит её, когда её
  ...
  НЕИЗВЕСТНЫМ, а не выполненным или невыполненным. Промпт на ход называет свой
  бюджет и в символах, а не только в словах, прямо говорит, что более длинный
  ответ обрезается и отбрасывается, просит вместо копии длинного результата
  инструмента указать число записей и сохранить значимые и называет
  единственный ожидаемый ключ: модель, перечислявшая каждую запись длинного
  результата, писала ответ, который обрезал лимит вывода, а обрезанный ответ не
  разбирается никогда. Обе инструкции — шаблоны
  ...
  вызов тратится на то, чтобы узнать, что саммари не меньше оригинала; соседние
  юниты, каждый из которых меньше порога, объединяются до
  `compaction_summary_group_max_tokens` и суммаризируются одним вызовом, так
  что история из одних коротких раундов всё же сжимается),
  словарный бюджет в промпте масштабируется от размера юнита, а не задан
  фиксированным числом предложений, и ограничен тем, что вмещает лимит вывода
  при `compaction_summary_output_tokens_per_word` (четыре: английская оценка в
  два токена на слово не учитывает ни JSON-экранирование, ни нелатинское
  письмо, а бюджет, посчитанный по ней, возвращается обрезанным, не разбирается
  и не фиксируется) за вычетом `compaction_summary_envelope_tokens` на сам
  JSON вокруг слов и не больше того, что примет `maxLength` грамматики, вызовы
  уходят по
  ...
  Вызов, провалившийся по причине, принадлежащей самому юниту, — запрос не
  помещается в окно суммаризатора или в ответе нет читаемого саммари, потому
  что лимит вывода обрезал конверт, — записывается на ЭТОТ юнит в
  `CompactionState.failed_anchor_keys`, и после
  `compaction_summary_failed_unit_max_attempts` обычный проход перестаёт его
  отправлять; у яруса свёртки остаётся своя попытка. Больше не засчитывается
  ничто: сбой транспорта (лимит частоты, 5xx, перезапуск суммаризатора) не
  говорит о юните ничего, как и саммари, просто оказавшееся не меньше
  оригинала. Остальные юниты пакета фиксируются в любом случае, поэтому один
  непосильный для суммаризатора юнит больше не обнуляет весь проход.
  Принудительные проходы перепись игнорируют и пробуют каждый юнит: они идут
  тогда, когда альтернатива — конец прогона. Перепись едет в снимке прогона, а
  записи, юнит которых ушёл из транскрипта, вычищаются в конце каждого прохода,
  так что она не растёт бесконечно.

  Отдельно от переписи проход списывается с бюджета повторов, ограниченного
  `compaction_failed_max_retries`. Списывается только проход, который пытался
  и не смог, — ярус выбросил исключение или вызов суммаризатора был сделан и
  ничего не дал, — и списывается один раз, сколько бы ярусов ни упало. Обычные
  и проактивные проходы делят `CompactionState.retry_count`; реактивный проход
  после отказа провайдера ведёт свой `CompactionState.reactive_retry_count`,
  потому что только этот профиль может уплотнять засеянную историю, и провалы
  проактивных проходов о нём ничего не доказывают. Прогресс любого профиля
  обнуляет оба счётчика — следующий проход любого вида видит уже другую
  историю, — и `rearm()` обнуляет их тоже. Сбой транспорта поэтому стоит
  проходу один повтор, а юниту ничего; сбой, принадлежащий юниту,
  засчитывается ещё и юниту. Принудительные проходы перепись игнорируют в
  любом случае. Оба счётчика едут в снимке прогона.

  Проактивный проход (обычный, аварийный на старте хода, поитерационный)
  решается до того, как открыться. `ContextManager.has_proactive_work`
  спрашивает каждый ярус, изменил бы он что-нибудь в проактивном профиле, ничего
  не меняя; если ни один не изменил бы — обычный случай: история из засеянных
  ходов, — гейт не открывает транзакцию вовсе: ни `COMPACTING`, ни событий,
  хуков, строки расхода или снимка. Движок запоминает эту пробу вместе с
  историей и константами, которые она видела, и не спрашивает снова, пока не
  изменится одно из них. За пределом бюджета проактивный проход не завершает
  прогон: провайдер ещё ничего не отклонил, поэтому проактивные ярусы
  суммаризатора приостанавливаются (изменение состояния
  `compaction_exhausted_proactive_suspended` при каждом исчерпании бюджета
  проактивным проходом; `retry_count` не сбрасывается, так что после
  приостановки один неудачный проход приостанавливает снова), и запрос уходит. Ярус 1 не
  нуждается в LLM и продолжает работать во время приостановки. Приостановка
  кончается через `compaction_proactive_suspension_iterations` визитов гейта
  или когда промпт вырос на `compaction_proactive_suspension_growth_ratio` от
  своего размера в момент приостановки — что наступит раньше. Отказ по
  контексту снимает её сразу — отказ провайдера или локальной подгонки по
  оценке либо точному подсчёту, оба запускают реактивный проход, — как и
  `rearm()` и возобновление из снимка, который её не несёт. Реактивный проход
  за пределом своего бюджета передаёт ход лестнице лимита вывода, пока есть
  меньший лимит, и проваливает прогон, только когда его нет.

  Обычный поитерационный гейт также отступает на
  `compaction_no_gain_backoff_iterations` итераций после прохода, освободившего
  меньше `compaction_min_gain_ratio`; счётчик переживает посообщенческий сброс
  восстановления, который раньше обнулял его до того, как тот успевал что-то
  пропустить. Отступ кончается раньше, когда промпт вырос на
  `compaction_no_gain_backoff_growth_ratio` от своего размера в момент
  отступа, и при любом отказе по контексту.
